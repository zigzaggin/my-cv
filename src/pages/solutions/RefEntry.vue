<template>
    <entry title="Call method on arbitrary $ref object in Vue" meta="2/4/19" location="PAI" tags="Vue.js">
        <p>I am integrating Vue.js into a massive legacy application right now and am attempting to create base
            components
        ...
        <p>I have created a Vue.js component for our modals that can be inherited from to create sub-modals that will
            house all of their appropriate behavior.
            The parent modal component's only responsibility is to house the base layout for modals and handle
            visibility - they also deal with the browser history API.</p>
        <p>Child Modals inherited from modal gain a couple of key methods, primarily <code>.open()</code> and
            <code>.close()</code> - these methods then ensure that the visibility is appropriately set and the browser
            history is stacked correctly.
        </p>
        ...
        <p>So... how do we solve this? The immediate thought was to move the behavior into a method and handle it in the
            current Vue context.</p>
        <p>This has a major drawback though since we're going to do this on almost every button we create. We could put
            the behavior in a mixin that can access the current contexts $refs and then use it
            something like
            <code>:open-modal="ref('prompt')"</code>, but this is also sub-optimal because my team needs to know to call
            the ref method, and we're now mixing quotes.
        </p>
        ...
        <p>Now, all <code>pai-btn</code> needs to do is traverse the object structure interrogating the component's
            <code>$refs</code> until it finds one that matches the provided string.</p>
